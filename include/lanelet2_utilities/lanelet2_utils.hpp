...
public:
                                            ...
                                            boost::optional<const int64_t &> last_lanelet_id)
  {
    ...
    std::sort(lanelets_n_costs.begin(), lanelets_n_costs.end(), [](const auto &i, const auto &j)
              { return i.lanelet_cost < j.lanelet_cost; });
  ...
  static void splitLinestring(const BasicLineString2d &ls, const BasicPoint2d &pt, BasicLineString2d &line1, BasicLineString2d &line2)
  {
    ...
    if (arc_pt.length < arc_nearest_pt.length && idx != ls.begin())
      idx--;
