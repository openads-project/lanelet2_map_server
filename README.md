...
- In manual mode, when switching to a new `map_filepath` without `origin_lat`/`origin_lon` in the same request and no origin was set before, the node derives the origin from the map's lower-left corner (`min_lat`, `min_lon`).
